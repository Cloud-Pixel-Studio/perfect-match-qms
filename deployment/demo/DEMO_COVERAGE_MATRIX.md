...
| `menu_pm_qms_iso9001_transition_actions` — Transition Actions | `pm.qms.iso9001.transition.action` | two actions generated from the completed Apex 2015-to-2026 gap assessment; one in progress and one draft | immutable gap snapshots, owner, target date, priority, guided implementation project | Quality Manager manages; QMS users/viewers read per ACL | exact two-action set, partial/gap snapshots, project links |
| `menu_pm_qms_iso9001_transition_reviews` — Transition Readiness Reviews | `pm.qms.iso9001.transition.review` | submitted Apex review; decision is to continue controlled actions | completed gap assessment, immutable action snapshot, independent Quality Supervisor reviewer, guided implementation project | Quality Manager prepares/submits; independent QMS manager reviews; users/viewers read | exact submitted review, two open actions, reviewer and project alignment |\n| `menu_pm_qms_objectives` — Objectives | `pm.qms.objective` | APEX-OBJ-001 | first-pass/dimensional quality KPI and owner | Quality Manager / Management User read | model count |
